"""
DAP Data Training Pipeline

This script provides a command-line interface for training various neural network models
on DAP cohort multi-omic data, including microbiome, metabolome, and methylation data.
...
Usage:
    python dap_train_all_models.py --model <model_name>

Example:
    python dap_train_all_models.py --model d
    python dap_train_all_models.py --model pdp
"""
...
def main():
    ...
    omic1 = "microbiome"
    omic2 = "metabolome"
    phenotype_col = "age_at_stool_collection"
    run_identifier = "dap"  # Identifier for the run, can be used to save results in a specific folder
    output_path = "."
    num_processes = 10
    quiet_mode = False
    ...
    if model == 'd':
        d.grid_search(param_grid=params_for_single_network,
                    get_data_func=dap_data_utils.get_data,
                    omic1=omic1,
                    omic2=omic2,
